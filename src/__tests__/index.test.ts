...
const mockOptions = {
  filePath: path.join(__dirname, '../__mocks__/eslint.config.mjs'),
  text: 'const foo=1;',
  eslintOptions: {},
  prettierOptions: {},
};

test('analyze formats JavaScript code correctly', async () => {
  const result = await analyze(mockOptions);
  assert.strictEqual(typeof result, 'object');
  assert.strictEqual(typeof result.output, 'string');
  assert.ok(Array.isArray(result.messages));
});

// test('analyze applies eslint fixes when eslint is enabled', async () => {
//   const options = { ...mockOptions, eslintOptions: { fix: true } };
//   const result = await analyze(options);
//   assert.ok(result.output.includes('const foo = 1;'));
// });

// test('analyze applies prettier formatting when enabled', async () => {
//   const options = { ...mockOptions, prettierOptions: { semi: false } };
//   const result = await analyze(options);
//   assert.ok(!result.output.includes(';'));
// });

// test('analyze respects prettierLast flag', async () => {
//   const options = { ...mockOptions, prettierLast: true };
//   const result = await analyze(options);
//   assert.ok(result.output.includes('const foo = 1'));
// });

// test('analyze returns only Prettier formatting for non-eslint files', async () => {
//   const options = { ...mockOptions, filePath: './test.txt' };
//   const result = await analyze(options);
//   assert.strictEqual(typeof result.output, 'string');
...
// // function getESLintConfigWithDefaultRules(overrides: Linter.RulesRecord =  {} as Linter.RulesRecord) {
// //   return [{
